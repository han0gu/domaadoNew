...
import com.jameshong.camerakit.core.R;
...
import static com.jameshong.camerakit.CameraKit.Constants.FLASH_AUTO;
import static com.jameshong.camerakit.CameraKit.Constants.FLASH_OFF;
import static com.jameshong.camerakit.CameraKit.Constants.FLASH_ON;
import static com.jameshong.camerakit.CameraKit.Constants.FLASH_TORCH;
...
public class CameraView extends com.jameshong.camerakit.CameraViewLayout {
    ...
    public CameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                mTimerShutter = a.getInteger(R.styleable.CameraView_ckTimerShutter, CameraKit.Defaults.DEFAULT_TIMERSHUTTER);
                mTimerCountdown = a.getInteger(R.styleable.CameraView_ckTimerCountdown, CameraKit.Defaults.DEFAULT_TIMERCOUNTDOWN);
            } finally {
                a.recycle();
            }
        }
        ...
        setTimerShutter(mTimerShutter);
        setTimerCountdown(mTimerCountdown);
    ...
    public boolean isFacingFront() {
        return mFacing == FACING_FRONT;
    }

    public boolean isFacingBack() {
        return mFacing == FACING_BACK;
    }
    ...
    public void setTimerShutter(int timerShutter) {
        this.mTimerShutter = timerShutter;
        mCameraImpl.setTimerShutter(mTimerShutter);
    }

    public int getTimerShutter() {
        return mCameraImpl != null ? mCameraImpl.getTimerShutter() : CameraKit.Defaults.DEFAULT_TIMERSHUTTER;
    }

    public int getTimerCountdown() {
        return mCameraImpl != null ? mCameraImpl.getTimerCountdown() : CameraKit.Defaults.DEFAULT_TIMERCOUNTDOWN;
    }

    public void setTimerCountdown(int timerCountdown) {
        this.mTimerCountdown = timerCountdown;
        mCameraImpl.setTimerCountdown(mTimerCountdown);
    }
    ...
    public void captureImageTimer(final CameraKitEventCallback<CameraKitImageTimer> callback) {
        mCameraImpl.captureImage(new CameraImpl.ImageCapturedCallback() {
            @Override
            public void imageCaptured(byte[] jpeg) {
                PostProcessor postProcessor = new PostProcessor(jpeg);
                ...
                CameraKitImageTimer image = new CameraKitImageTimer(postProcessor.getJpeg());
                if (callback != null) callback.callback(image);
                mEventDispatcher.dispatch(image);
            }
        });
    ...
    public void captureImageCountdown(int mTimerCountdown) {
        CameraKitImageTimer image = new CameraKitImageTimer(null);
        image.setCountDown(mTimerCountdown);
        mEventDispatcher.dispatch(image);
    }
